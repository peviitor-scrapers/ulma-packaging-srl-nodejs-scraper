...
This scraper extracts job listings from ULMA PACKAGING S.R.L. careers page and imports them to peviitor.ro.

Target: https://www.ulmapackaging.ro/lucreaza-cu-noi/
...
1. **Start with brand** - We know the brand (e.g., "ulmapackaging")
...
8. **Scrape new jobs** - Extract jobs from ULMA careers page
...
3. **Scrape jobs** - Extract jobs from ULMA careers page
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape ULMA careers page (jobs for Romania)
    │
    ▼
